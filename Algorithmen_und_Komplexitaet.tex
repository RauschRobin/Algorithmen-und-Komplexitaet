...
\begin{minipage}[c]{0.4\textwidth}
	\begin{center}
		\includegraphics[scale=0.25]{Bilder/quicksort.jpeg}		
	\end{center}
\end{minipage}
\vspace*{0.7cm}
\newline
...
\subsection{Heapsort}
	...
	\begin{minipage}[c]{0.7\textwidth}
		\begin{enumerate}
			\item Liste in Binärbaum übertragen
			\item Solange Binärbaum nicht leer:
			\begin{enumerate}
				\item Von unten nach oben Heapeigenschaften herstellen
				\item Von oben nach unten Heapeigenschaften kontrollieren
				\item Wurzel mit der letzten Stelle des Heaps (unterste -> rechteste) tauschen und in neuen (sortierten) Array schreiben, weil diese nun Sortiert ist
			\end{enumerate}
			%Sein Skript sagt so:
			%\item Transformiere A in Max-Heap (heapify)
			%\item Vertausche Wurzel ($a_0$) mit letztem Element des Arrays (Heap endet jetzt mit vorletztem Element)
			%\item Lasse neue Wurzel herabsinken (bubble down)
			%\item Vertausche neue Wurzel mit vorletztem Element (Heap endet jetzt mit drittletztem Element)
			%\item Lasse neue Wurzel herabsinken
			%\item ...
		\end{enumerate}
	\end{minipage}
	\begin{minipage}[c]{0.3\textwidth}
		\begin{center}
			\begin{tikzpicture}[scale=0.6]
				...
				\node[draw, draw=green, circle] at (0,-2){5};

				\node[draw, draw=green, circle] at (1.5,-4){8};
				\node[draw, circle] at (-1.5,-4){3};

				
				\node[draw, draw=green, circle] at (3,-6){9};
				\node[draw, circle] at (-3,-6){1};

				% new tree
				\node[draw, circle] at (0,-8){9};

				\node[draw, draw=green, circle] at (1.5,-10){5};
				\node[draw, circle] at (-1.5,-10){3};

				
				\node[draw, draw=green, circle] at (3,-12){8};
				\node[draw, circle] at (-3,-12){1};

				% new tree
				\node[draw, draw=red, circle] at (0,-14){9};

				\node[draw, circle] at (1.5,-16){8};
				\node[draw, circle] at (-1.5,-16){3};

				
				\node[draw, circle] at (3,-18){5};
				\node[draw, circle] at (-3,-18){1};

				% new tree
				\node[draw, circle] at (0,-20){5};

				\node[draw, circle] at (1.5,-22){8};
				\node[draw, circle] at (-1.5,-22){3};

				\node[draw, circle] at (-3,-24){1};

				\draw[green,very thick](-2.5,-25.5) rectangle (2.5,-26.5);
				\draw[green, very thick](-1.5,-25.5) -- (-1.5,-26.5);
				\draw[green, very thick](-0.5,-25.5) -- (-0.5,-26.5);
				\draw[green, very thick](0.5,-25.5) -- (0.5,-26.5);
				\draw[green, very thick](1.5,-25.5) -- (1.5,-26.5);
				\node at (-2,-26){9};
